# CB 循环任务 v2：1.20.1 → 1.20.4 走廊达标 ✅ 【已完成，周期 19 收官】

> **最终战果**：@1.20.2 闭合（未记账 19 / 94.8%）+ @1.20.4 闭合（未记账 35 / 91.1% / 账目 1507）；双版本实机验收 "Done"；八种改写机制 + 记账制度 + shim 前移流程全部实战验证。周期 19：Block.onBreak 双向桥（空体重载 + TAIL 注入恢复子类覆写）、getScoreboardTeam 返回型桥、走廊前尸体分类、1.20.3 网络/rework 波记账。
...
- **周期 18（C4 开工：1.20.4 管线全套立起）**：1.20.4 stub（7787 类）；**编译器当检测器**做 shim 前移——1.20.3 codec 波实锤：class_195/class_5267 删除（facade 化，class_195 完整 facade 含老构造器/getId/toJson 最小实现）、谓词 fromJson 统片删除（7 静态桥+5 覆盖移入 ledger）、Advancement toJson/method_765 再砍（method_709/698 ledger）、Ingredient.toJson 消失（IngredientBridge 退役）。1.20.4 桥编译绿、20KB/13 mixin+facade×3。首扫 @1.20.4：**未记账 357 / 已记账 935 / server-clean 175/270=64.8%**——1.20.3 墙的新工单浮出，下周期 audit@1.20.4 开批次清剿至闭合，然后 server-1.20.4 验收。
